...
C_QCD::C_QCD(TString Name_, TString id_):
  Selection(Name_,id_)
{
}
...
C_QCD::~C_QCD(){
  for(int j=0; j<Npassed.size(); j++){
    std::cout << "C_QCD::~C_QCD Selection Summary before: " 
	 << Npassed.at(j).GetBinContent(1)     << " +/- " << Npassed.at(j).GetBinError(1)     << " after: "
	 << Npassed.at(j).GetBinContent(NCuts) << " +/- " << Npassed.at(j).GetBinError(NCuts) << std::endl;
  }
  std::cout << "C_QCD::~C_QCD()" << std::endl;
}

void  C_QCD::Configure(){
  // Setup Cut Values
