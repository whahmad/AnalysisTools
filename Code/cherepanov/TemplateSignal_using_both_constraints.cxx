#include "TemplateSignal_using_both_constraints.h"
...
TemplateSignal_using_both_constraints::TemplateSignal_using_both_constraints(TString Name_, TString id_):
  Selection(Name_,id_)
{
}

TemplateSignal_using_both_constraints::~TemplateSignal_using_both_constraints(){
  for(int j=0; j<Npassed.size(); j++){
    std::cout << "TemplateSignal_using_both_constraints::~TemplateSignal_using_both_constraints Selection Summary before: " 
	 << Npassed.at(j).GetBinContent(1)     << " +/- " << Npassed.at(j).GetBinError(1)     << " after: "
	 << Npassed.at(j).GetBinContent(NCuts) << " +/- " << Npassed.at(j).GetBinError(NCuts) << std::endl;
  }
  std::cout << "TemplateSignal_using_both_constraints::~TemplateSignal_using_both_constraints()" << std::endl;
}

void  TemplateSignal_using_both_constraints::Configure(){
  // Setup Cut Values
...
void  TemplateSignal_using_both_constraints::Store_ExtraDist(){
...
void  TemplateSignal_using_both_constraints::doEvent(){
  ...
  if(status){
    ...
    TLorentzVector tau_3pi_vis = Ntp->KFTau_TauVis_p4(HighestPtTauIndex);
    ...
    xpi = tau_3pi_vis.P()/tau_3pi.P();
    xmu = xpi*muon.Pt()/tau_3pi_vis.Pt();
  ...
  for(int iz =0; iz<Ntp->NMCSignalParticles(); iz++){
   ...
   TruthPi.SetXYZM(0,0,0,0);
    if(signal){
	  ...
	  TruthPi += Ntp->MCTauandProd_p4(TauPiIndex,iProd1);
	}


      }
	xpiTruth = TruthPi.P()/TruthTauPi.P();
	xmuTruth = xpiTruth*TruthMu.Pt()/TruthPi.Pt();

	
	
	xmuon_plus_truth.at(t).Fill(xmuTruth,w*Ntp->TauSpinerGet(TauSpinerInterface::hplus)*UnSpin_WT);
	xmuon_minus_truth.at(t).Fill(xmuTruth,w*Ntp->TauSpinerGet(TauSpinerInterface::hminus)*UnSpin_WT);
