#include "TemplateSignal_Using_Muon_angleNoPU.h"
...
TemplateSignal_Using_Muon_angleNoPU::TemplateSignal_Using_Muon_angleNoPU(TString Name_, TString id_):
  Selection(Name_,id_)
{
}

TemplateSignal_Using_Muon_angleNoPU::~TemplateSignal_Using_Muon_angleNoPU(){
  for(int j=0; j<Npassed.size(); j++){
    std::cout << "TemplateSignal_Using_Muon_angleNoPU::~TemplateSignal_Using_Muon_angleNoPU Selection Summary before: " 
	 << Npassed.at(j).GetBinContent(1)     << " +/- " << Npassed.at(j).GetBinError(1)     << " after: "
	 << Npassed.at(j).GetBinContent(NCuts) << " +/- " << Npassed.at(j).GetBinError(NCuts) << std::endl;
  }
  std::cout << "TemplateSignal_Using_Muon_angleNoPU::~TemplateSignal_Using_Muon_angleNoPU()" << std::endl;
}

void  TemplateSignal_Using_Muon_angleNoPU::Configure(){
  // Setup Cut Values
  ...
  xmuonT_plus_MC=HConfig.GetTH1D(Name+"_xmuonT_plus_MC","xmuonT_plus_MC",15,0,2,"xmuonT_plus_MC","Events");
  xmuonT_minus_MC=HConfig.GetTH1D(Name+"_xmuonT_minus_MC","xmuonT_minus_MC",15,0,2,"xmuonT_minus_MC","Events");
...
void  TemplateSignal_Using_Muon_angleNoPU::Store_ExtraDist(){
...
void  TemplateSignal_Using_Muon_angleNoPU::doEvent(){
  ...
  if(status){
    ...
    xmuon_plus.at(t).Fill(muon.E()/tau.E(),w*Ntp->TauSpinerGet(TauSpinerInterface::hplus)*UnSpin_WT);
    xmuon_minus.at(t).Fill(muon.E()/tau.E(),w*Ntp->TauSpinerGet(TauSpinerInterface::hminus)*UnSpin_WT);
    xmuonT_plus.at(t).Fill(muon.Et()/tau.Et(),w*Ntp->TauSpinerGet(TauSpinerInterface::hplus)*UnSpin_WT);
    xmuonT_minus.at(t).Fill(muon.Et()/tau.Et(),w*Ntp->TauSpinerGet(TauSpinerInterface::hminus)*UnSpin_WT);
  ...
  for(int iz =0; iz<Ntp->NMCSignalParticles(); iz++){
    ...
    if(signal){
      ResolMu.at(t).Fill((tau.E() -TruthTauMu.E()),w); 
      ResolMuT.at(t).Fill((tau.Et() -TruthTauMu.Et()),w); 
    }
    ...
    if(signal){
      ...
      thetaTruth = muon.Theta();
      if(sin(thetaTruth)!=0){
	energyTruth = sqrt(mtau*mtau*sin(thetaTruth)*sin(thetaTruth) + TruthTauPi.Pt()*TruthTauPi.Pt())/sin(thetaTruth);
      ...
      RecoiledTauMu.SetXYZM(-TruthTauPi.Px(), -TruthTauPi.Py(),sqrt(energyTruth*energyTruth - mtau*mtau)*cos(thetaTruth),mtau);
      
      
      xmuon_plus_truth.at(t).Fill(TruthMu.E()/RecoiledTauMu.E(),w*Ntp->TauSpinerGet(TauSpinerInterface::hplus)*UnSpin_WT);
      xmuon_minus_truth.at(t).Fill(TruthMu.E()/RecoiledTauMu.E(),w*Ntp->TauSpinerGet(TauSpinerInterface::hminus)*UnSpin_WT);
      xmuon_plus_MC.at(t).Fill(TruthMu.E()/TruthTauMu.E(),w*Ntp->TauSpinerGet(TauSpinerInterface::hplus)*UnSpin_WT);
      xmuon_minus_MC.at(t).Fill(TruthMu.E()/TruthTauMu.E(),w*Ntp->TauSpinerGet(TauSpinerInterface::hminus)*UnSpin_WT);
      
      xmuonT_plus_truth.at(t).Fill(TruthMu.Et()/RecoiledTauMu.Et(),w*Ntp->TauSpinerGet(TauSpinerInterface::hplus)*UnSpin_WT);
      xmuonT_minus_truth.at(t).Fill(TruthMu.Et()/RecoiledTauMu.Et(),w*Ntp->TauSpinerGet(TauSpinerInterface::hminus)*UnSpin_WT);
      xmuonT_plus_MC.at(t).Fill(TruthMu.Et()/TruthTauMu.Et(),w*Ntp->TauSpinerGet(TauSpinerInterface::hplus)*UnSpin_WT);
      xmuonT_minus_MC.at(t).Fill(TruthMu.Et()/TruthTauMu.Et(),w*Ntp->TauSpinerGet(TauSpinerInterface::hminus)*UnSpin_WT);
      
      ResolMuTruth.at(t).Fill(RecoiledTauMu.E() -TruthTauMu.E(),w); 
      ResolMuTruthT.at(t).Fill(RecoiledTauMu.Et() -TruthTauMu.Et(),w); 
 
      
		 printf("No PU---------->>>PU reweigh: %f \n",w);
