...
void plotting(){
	...
	TH1D* zjetpt_signal = getHisto("zjetptMC_emu_DY",mcscale.at(mcscale.size()-1),0,infile,syst.at(syst.size()-1));
	TH1D* zjetpt_dy = getHisto("zjetptMC_DY",mcscale.at(mcscale.size()-2),2345,infile,syst.at(syst.size()-2));
	zjetpt_signal->Scale(1./zjetpt_signal->Integral());
	zjetpt_dy->Scale(1./zjetpt_dy->Integral());
	TH1D* zjetpt_ratio = getDataMC(zjetpt_signal,zjetpt_dy);
	//drawPlot(zjetpt_signal, zjetpt_dy, zjetpt_ratio, "Custom MC", "Official MC", "Single Jet Pt", "GeV");

	TH1D* znjets_signal = getHisto("znjetsMC_emu_DY",mcscale.at(mcscale.size()-1),0,infile,syst.at(syst.size()-1));
	TH1D* znjets_dy = getHisto("znjetsMC_DY",mcscale.at(mcscale.size()-2),2345,infile,syst.at(syst.size()-2));
	znjets_signal->Scale(1./znjets_signal->Integral());
	znjets_dy->Scale(1./znjets_dy->Integral());
	TH1D* znjets_ratio = getDataMC(znjets_signal,znjets_dy);
	//drawPlot(znjets_signal, znjets_dy, znjets_ratio, "Custom MC", "Official MC", "Single Jet Pt", "GeV");

	TH1D* zmet_signal = getHisto("zmetMC_emu_DY",mcscale.at(mcscale.size()-1),0,infile,syst.at(syst.size()-1));
	TH1D* zmet_dy = getHisto("zmetMC_DY",mcscale.at(mcscale.size()-2),2345,infile,syst.at(syst.size()-2));
	zmet_signal->Scale(1./zmet_signal->Integral());
	zmet_dy->Scale(1./zmet_dy->Integral());
	TH1D* zmet_ratio = getDataMC(zmet_signal,zmet_dy);
	//drawPlot(zmet_signal, zmet_dy, zmet_ratio, "Custom MC", "Official MC", "Single Jet Pt", "GeV");
	...
	TH1D* zeta_dy = getHisto("zetaMC_DY",1,2345,infile,0);
	...
	TH1D* zpt_dy = getHisto("zptMC_DY",1,2345,infile,0);
