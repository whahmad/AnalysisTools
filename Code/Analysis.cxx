...
int main() {
	...
	for (unsigned int i = 0; i < UncertType.size(); i++) {
		...
		for (unsigned int j = 0; j < Analysis.size(); j++) {
			...
			selections.push_back(
					SF.Factory(Analysis.at(j), UncertType.at(i), mode, runtype,
							Lumi));
		}
	}
	...
	if (mode == Selection_Base::ANALYSIS) {
		...
		for (i = 0; i < nentries; i++) {
			...
			num++;
			if (num >= 10000) {
				cout << "Starting event:" << i << " out of " << nentries
						<< endl;
				num = 0;
			}
			Ntp.Get_Event(i);
		...
		if (skim)
			Ntp.SaveCloneTree();
		if (ListOfFilesRead.at(0)
				== ListOfFilesRead.at(ListOfFilesRead.size() - 1))
			ListOfFilesRead.pop_back();
		if (i == nentries && Files.size() == ListOfFilesRead.size())
			cout << " Number of events Read: " << i << " out of " << nentries
					<< " SUCCESSFULL" << endl;
		else
			cout << " Number of events Read: " << i << " out of " << nentries
					<< " Number of files Read: " << ListOfFilesRead.size()
					<< " out of " << Files.size() << " FAILED" << endl;
		...
		for (k = 0; k < ListOfFilesRead.size(); k++) {
			cout << " Number of events: " << EventsReadFromFile[k]
					<< " from file: " << ListOfFilesRead[k] << endl;
		}
			...
			for (unsigned int j = 0; j < selections.size(); j++) {
				if (selections.at(j)->Get_SysType() == "default") {
					for (unsigned int i = 0; i < UncertList.size(); i++) {
						...
						cout << "Adding Systematic Uncertainty "
								<< UncertType.at(i) << endl;
						Selection_Base *s = SF.Factory(
								selections.at(j)->Get_Analysis(),
								UncertList.at(i), mode, runtype, Lumi);
						s->LoadResults(Files);
	...
	if (mode != Selection_Base::RECONSTRUCT) {
		...
		cout << "Time in Ntuple Loop: " << hour << "hr " << min << "min " << sec
				<< "s" << endl;
	...
	cout << "Run Time: " << hour << "hr " << min << "min " << sec << "s"
			<< endl;
