...
if( $ARGV[0] eq "--TauNtuple"){
   ...
   #Tau Package recomendation for 5_3_X (X>=12): nothing to do
	...
	# Ntuple code
    system(sprintf("echo \"git clone https://github.com/bkargoll/TauDataFormat TauDataFormat; cd TauDataFormat; git checkout; cd ../; \" >> Install_TauNtuple_$CMSSWRel-$time"));
    system(sprintf("echo \"git clone https://github.com/bkargoll/SkimProduction SkimProduction; cd SkimProduction; git checkout; cd ../; \" >> Install_TauNtuple_$CMSSWRel-$time"));
