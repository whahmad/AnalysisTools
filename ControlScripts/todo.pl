...
if( $ARGV[0] eq "--GRID" ){
	...
	system(sprintf("echo \"rm junk \" >> $OutputDir/workdir$set/Submit")) ;
	system(sprintf("echo \"touch junk \" >> $OutputDir/workdir$set/Submit")) ;
	...
	system(sprintf("echo \"  srmls -count=999 -offset=\\\$filecount -recursion_depth=2 srm://$gridsite:8443/pnfs/physik.rwth-aachen.de/cms/store/user/$UserIDCern/workdir$set/ >& junk \" >> $OutputDir/workdir$set/Submit")) ;
	system(sprintf("echo \"  let filecount=filecount+999 \" >> $OutputDir/workdir$set/Submit")) ;
	system(sprintf("echo \"  readfiles=`cat junk | wc -l`; \" >> $OutputDir/workdir$set/Submit")) ;
	...
	foreach $DS (@DataSets){
		...
		foreach $file (@files){
			...
			system(sprintf("echo \"isSkim=\\\$(cat junk | grep SKIMMED_NTUP_$B.root  | wc -l) \n if [ \\\$isSkim == 1 ]; then \" >> $OutputDir/workdir$set/Submit "));
