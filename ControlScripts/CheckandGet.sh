...
if [ "${1}"  == "--help" ]; then
    ...
    echo "--detailed                Prints out the status of each job"
fi
if [ "${1}"  == "--detailed" ]; then
...
touch jobs_complete
touch jobs_get
cat jobs_submitted | awk '{system("glite-wms-job-status " $1 " | grep -i done | wc -l |awk \x27{sum++}END{if($1 == 1) print  \" glite-wms-job-output --dir "$2 " " $1 " ; mv " $2 "/$USER*/* "$2 "; rm -rf " $2 "/$USER* ; echo " $1 " " $2 "  >> jobs_complete  \"}\x27 >> jobs_get ")}'
source jobs_get 
rm jobs_get
